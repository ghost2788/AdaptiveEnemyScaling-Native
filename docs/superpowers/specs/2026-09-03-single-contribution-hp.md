...
Gate1 update (2026-09-03): catalog accepted by the user after functional proof
and three observational memory samples per condition. User accepted the
disclosed lack of controlled load-time measurements/configuration parity and
will monitor perceived performance during play. This does not waive wounded
migration, interrupted-recovery, or production smoke-test requirements.
...
Current checkpoint update (2026-09-03): UI02 records accepted catalog functional
qualification and disclosed cost-measurement limits. UI03 records the controlled
two-sample wounded replacement, legacy/total save-load retention and cleanup
proof. Those primitive results now support planning integration. They do not
prove normal self-healing interactions, intervention detection, interrupted
recovery, mixed ownership or production safety. New wounded screenshots remain
outstanding; text confirmations and parsed save facts do not satisfy the full
visual-evidence requirement. Historical pre-qualification statements below are
superseded only for the exact capabilities named in this update.
